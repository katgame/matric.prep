...
import { TutorRunner } from "@/components/tutor-runner";
import { fetchPaperTutor } from "@/lib/api";
...
export default async function TutorPage({ params }: PageProps) {
  ...
  let paper;
  try {
    paper = await fetchPaperTutor(paperId);
  } catch {
    return (
      <div className="flex min-h-full flex-col">
        <AppHeader backHref={`/papers/${paperId}`} backLabel="Back to paper" title="AI workspace" />
  ...
  return (
    <div className="flex min-h-full flex-col">
      <AppHeader backHref={`/papers/${paper.id}`} backLabel="Back to paper" title="AI workspace" />
      <TutorRunner paper={paper} />
    </div>
  );
}
